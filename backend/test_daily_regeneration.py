#!/usr/bin/env python3
"""
Test daily plan regeneration only
"""
...
async def test_daily_regeneration():
    print("🔍 Testing daily plan regeneration...")
    ...
    try:
        ...
        if daily_plan:
            ...
            # Show original first meal
            if daily_plan.content and "meals" in daily_plan.content:
                original_meal = daily_plan.content["meals"][0]
                print(f"📋 Original breakfast: {original_meal.get('name', 'Unknown')}")
            ...
            try:
                ...
                # Show new first meal
                if regenerated_plan.content and "meals" in regenerated_plan.content:
                    new_meal = regenerated_plan.content["meals"][0]
                    print(f"🍳 New breakfast: {new_meal.get('name', 'Unknown')}")
...
if __name__ == "__main__":
    asyncio.run(test_daily_regeneration())