...
interface WeeklyPlanViewProps {
  ...
  onRegenerateDay?: (dayIndex: number, useML: boolean) => void;
  onRegenerateWeek?: (useML: boolean) => void;
  ...
  return (
    <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
          ...
          {showRegenerate && onRegenerateWeek && (
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              {/* <button
                onClick={() => onRegenerateWeek(false)}
              ...
              <button
                onClick={() => onRegenerateWeek(true)}
                disabled={isRegeneratingWeek}
                className="flex items-center justify-center px-3 py-2 text-sm font-medium text-purple-700 dark:text-purple-300 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border border-purple-300 dark:border-purple-700 rounded-md hover:from-purple-100 hover:to-indigo-100 dark:hover:from-purple-900/30 dark:hover:to-indigo-900/30 disabled:opacity-50 disabled:cursor-not-allowed transition-colors w-full sm:w-auto"
      ...
      {selectedDay && (
        <DailyPlanView
          plan={selectedDay}
          onRegenerateMeal={onRegenerateMeal ? (mealType, useML) => onRegenerateMeal(selectedDayIndex, mealType, useML) : undefined}
          onRegenerateDay={onRegenerateDay ? (useML) => onRegenerateDay(selectedDayIndex, useML) : undefined}
          regeneratingMeal={
