...
export const BalanceGuidance: React.FC<BalanceGuidanceProps> = ({ guidance, className = '' }) => {
  const isWarning = guidance.type === 'warning';
  ...
  const iconColor = isWarning ? 'text-amber-500 dark:text-amber-400' : 'text-blue-500 dark:text-blue-400';

  return (
    <div className={`rounded-2xl border p-4 ${bgColor} ${className}`}>
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          <h4 className={`text-sm font-semibold ${textColor}`}>
            {isWarning ? 'Balance Suggestion' : 'Optional Balance Tips'}
          </h4>
          <p className={`mt-1 text-sm ${textColor} leading-relaxed`}>
            {guidance.message}
          </p>
